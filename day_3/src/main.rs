use std::{fmt, collections::{HashMap, HashSet}, cmp};
...
.....................244....6.....................................789......................687..............................................";
    ...
    let values = input.lines()
        ...
        .map(|(line_num, line)| {
            ...
            line.trim().chars()
                .enumerate()
                .scan(None, move |acc: &mut Option<IndexValue>, (char_index, c)| {
                    let curr = if c.is_numeric() {
                        match acc {
                            Some(x) => Some(Some(IndexValue {
                                self_index: (line_num, char_index),
                                index: (x.index.0, x.index.1),
                                value: c.to_digit(10).unwrap() + x.value * 10
                            })),
                            None => Some(Some(IndexValue {
                                self_index: (line_num, char_index),
                    ...
                    *acc = curr.clone().unwrap();
                    curr
                })
        }).flatten().filter_map(|v| v);
    ...
    let nums: HashSet<IndexValue> = input.lines()
        ...
        .map(|(line_num, line)| {
            ...
            line.chars()
                .enumerate()
                .filter_map(move |(char_num, c)| {
                    if c.is_numeric() || c == '.' {
                        None
                    } else {
                        Some((line_num, char_num))
                    }
                })
            .map(|(x, y)| {
                vec![
...
impl fmt::Display for IndexValue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Self Index: {:?}, Index: {:?}, Value: {}",self.self_index, self.index, self.value)
    }
}

impl cmp::PartialEq for IndexValue {
