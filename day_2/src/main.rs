...
fn main() {
    let input = "Game 1: 1 blue, 8 green; 14 green, 15 blue; 3 green, 9 blue; 8 green, 8 blue, 1 red; 1 red, 9 green, 10 blue
...
Game 100: 6 green, 15 red, 12 blue; 9 red; 16 red; 17 red, 3 blue, 7 green";
    ...
    let out: usize = input.lines()
        .map(|game| {
            let line = &game[game.find(':').unwrap() + 2..];
            let mut red_max = 0;
            let mut blue_max = 0;
            let mut green_max = 0;
            ...
            line.split(';')
                ...
                .for_each(|drawing| {
                    println!("{}", drawing);
                    let mut split = drawing.trim().split(" ");
                    let num: usize = split.next().unwrap().parse().unwrap();
                    let color = split.next().unwrap();

                    match color {
                        "red" => red_max = cmp::max(num, red_max),
                        "blue" => blue_max = cmp::max(num, blue_max),
                        "green" => green_max = cmp::max(num, green_max),
                        _ => ()
                    }
                });
